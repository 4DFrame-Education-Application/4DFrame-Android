...
//        uri?.let { currentIndex?.let { _ -> buildGalleryViewModel.selectImage(it) } }
//    }
//
...
class BuildGalleryActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        buildGalleryViewModel.navigateToNextPage.observe(this, Observer { shouldNavigate ->
            if (shouldNavigate) {
                buildGalleryViewModel.resetNavigation() //이동 전에 resetNavigation호출 / naviagtetonextpage값을 true로 설정한 후 이동이 완료된 후 fasle로 설정하여 다음번에도 작동하게 변경

//                buildGalleryViewModel.sendPhotosToServer(this) // 서버로 이미지 전송 추가
                moveToNextPage()
            }
        })
    }
